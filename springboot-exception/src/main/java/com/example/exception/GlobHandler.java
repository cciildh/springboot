...
/**
 * GlobHandler 异常处理 改方式在ctroller建议不使用try catch   如果需要请注意 Exception  类型RuntimeException
 */
...
public class GlobHandler {
    ...
    public Object runtimeException(RuntimeException e, HttpServletRequest req) {
        log.error("运行时异常:", e);
        return handlerException(e, req, "运行时异常:" + e.getMessage(), 500);
    }
    ...
    private Object handlerException(Exception e, HttpServletRequest req, String msg, int status) {
        ...
        if ((contentTypeHeader != null && contentTypeHeader.contains("application/json"))
            ...
            return ResponseMesg.errorMsg(status, msg);
